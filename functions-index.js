...
// ================================
// MIGRAR DEVOCIONALES VIEJOS A R2 ✅ MUY FÁCIL
// Se ejecuta desde el navegador
// ================================
exports.migrarDevocionalesViejosAR2 = onRequest(
  {
    cors: true,
    secrets: ["ADMIN_KEY", "R2_ACCESS_KEY_ID", "R2_SECRET_ACCESS_KEY"]
  },
  async (req, res) => {
    try {
      if (req.method === "OPTIONS") {
      ...
      if (req.method !== "GET") {
        return res.status(405).send("Usá GET");
      }

      const clave = String(req.query?.clave || "").trim();
      const dryRun = String(req.query?.dryRun || "true").trim().toLowerCase() === "true";
      ...
      const rootSnap = await admin.database().ref("devocionalesIglesia").get();
      const root = rootSnap.val() || {};

      const candidatos = [];

      for (const [uid, byTs] of Object.entries(root)) {
        if (!byTs || typeof byTs !== "object") continue;

        for (const [ts, item] of Object.entries(byTs)) {
      ...
      const seleccion = candidatos.slice(0, limit);
      ...
      for (const entry of seleccion) {
        const { uid, ts, oldUrl } = entry;

        try {
          resultado.procesados++;

          const nuevoNombre = `devocional_${ts}.png`;
          const key = `devocionales/${nuevoNombre}`;
          const nuevaUrl = `${publicBaseUrl}/${key}`;
          ...
          if (!resp.ok) {
            throw new Error(`No pude descargar imagen vieja (${resp.status})`);
          }
          ...
          await admin.database().ref(`devocionalesIglesia/${uid}/${ts}`).update({
            url: nuevaUrl,
            migradoAR2: true,
            oldUrlBackup: oldUrl,
            migradoTs: Date.now()
      ...
      console.error("migrarDevocionalesViejosAR2 error:", e);
      return res.status(500).json({
        error: String(e?.message || e)
      });
    }
